...
<img src="https://learnopencv.com/wp-content/uploads/2022/10/bike.png">

<small> Source: Pexels </small>

And then given the above image its semantically segmentated image would be the following

<img src="https://learnopencv.com/wp-content/uploads/2022/10/bike-segmented.png">
...
1. **Autonomous Driving**
  ...
  <img src="https://learnopencv.com/wp-content/uploads/2022/10/traffic-segmentation.png" width="600"/> <br/>
...
2. **Facial Segmentation**
  ...
  <img src="https://learnopencv.com/wp-content/uploads/2022/10/face-segmentation.jpg" width="600"/> <br/>
...
3. **Indoor Object Segmentation**
  ...
  <img src="https://learnopencv.com/wp-content/uploads/2022/10/indoor-object-segmentation.jpg" width="600"/><br/>
...
4. **Geo-Land Sensing**
  ...
  <img src="https://learnopencv.com/wp-content/uploads/2022/10/geo-sense-segmentation.jpg" width="600"/> <br/>
...
The provided Python notebook implements a **three-class semantic segmentation pipeline** to identify
and visually distinguish a **girl**, a **yellow bicycle**, and the **background** in an input image.
The process begins by loading the image using PIL and displaying it for visual confirmation.
It then applies standard preprocessing transformations—resizing, center cropping, tensor conversion,
and ImageNet normalization—to prepare the image for a **pretrained DeepLabV3 model** with a ResNet50
backbone, originally trained on the PASCAL VOC dataset (21 classes). The model performs inference
in evaluation mode on either CPU or GPU, producing per-pixel class predictions. From these predictions,
pixels labeled as "background" (class 0) and "person" (class 15) are mapped to custom classes 0 and 1,
 respectively.

To isolate the **yellow bicycle**, which is not explicitly a VOC class, the code employs **color-based
 segmentation in HSV space**. The original image is converted to HSV, and a robust yellow color range
  (Hue: 20–40, high saturation and value) is used to generate a binary mask. This mask is resized to
  match the model’s output resolution (224×224) using nearest-neighbor interpolation to preserve
  label integrity, followed by morphological opening to remove noise. Crucially, the bicycle mask
  is refined by excluding regions already classified as "person" to prevent overlap (e.g., yellow
                                                                                     clothing).
                                                                                     These yellow,
                                                                                     non-person
                                                                                      regions are
...
A **custom decode function** maps the final 3-class label map (0: background, 1: girl, 2: bicycle)
to an RGB visualization using distinct colors: black, light purple, and bright yellow.
This colored segmentation map is displayed standalone and then **overlaid semi-transparently**
 (α = 0.6) onto the original image after resizing the mask back to the input resolution.
 The result is a clean, interpretable visualization highlighting the girl in purple and
 the bicycle in yellow against the natural background.

Overall, the approach effectively combines **deep learning-based semantic understanding**
 (via DeepLabV3) with **classical color filtering** (HSV thresholding) to achieve targeted
  3-class segmentation beyond the limitations of the pretrained model's vocabulary.
  The modular design, use of morphological cleaning, and careful mask alignment demonstrate
   robust engineering for real-world composite object detection.